...
| Re-film one scene | `vidsmith retake <name> --scene N`, [configuration.md](docs/claude/configuration.md) | It rerolls the search, it cannot change the subject; a scene wrong twice needs its words rewritten; a clip that keeps coming back goes in `visuals.exclude` |
